CLASS zcl_dt_logger DEFINITION
  PUBLIC
  CREATE PRIVATE
  GLOBAL FRIENDS zcl_dt_logger_factory.
...
*"* do not include other source files here!!!
    ...
    INTERFACES zif_dt_logger.
    ALIASES: add FOR zif_dt_logger~add,
             a FOR zif_dt_logger~a,
             e FOR zif_dt_logger~e,
             w FOR zif_dt_logger~w,
             i FOR zif_dt_logger~i,
             s FOR zif_dt_logger~s,
             has_errors FOR zif_dt_logger~has_errors,
             has_warnings FOR zif_dt_logger~has_warnings,
             is_empty FOR zif_dt_logger~is_empty,
             length FOR zif_dt_logger~length,
             save FOR zif_dt_logger~save,
             export_to_table FOR zif_dt_logger~export_to_table,
             fullscreen FOR zif_dt_logger~fullscreen,
             popup FOR zif_dt_logger~popup,
             handle FOR zif_dt_logger~handle,
             db_number FOR zif_dt_logger~db_number,
             header FOR zif_dt_logger~header.
    ...
    CLASS-METHODS new
      ...
      RETURNING
        VALUE(r_log)    TYPE REF TO zcl_dt_logger.
...
*"* do not include other source files here!!!
    ...
    DATA settings           TYPE REF TO zif_dt_logger_settings.
    ...
    METHODS:
      ...
      add_structure
        ...
        RETURNING
          VALUE(self)   TYPE REF TO zif_dt_logger.
...
CLASS zcl_dt_logger IMPLEMENTATION.
  ...
  METHOD new.
    IF auto_save IS SUPPLIED.
      r_log ?= zcl_dt_logger_factory=>create_log(
        object = object
        subobject = subobject
        desc = desc
        context = context
        settings = zcl_dt_logger_factory=>create_settings(
  ...
  METHOD open.
    IF auto_save IS SUPPLIED.
      r_log ?= zcl_dt_logger_factory=>open_log(
        object = object
        subobject = subobject
        desc = desc
  ...
  METHOD zif_dt_logger~export_to_table.
  ...
  METHOD zif_dt_logger~fullscreen.
    DATA:
  ...
  METHOD zif_dt_logger~is_empty.
    rv_yes = boolc( length( ) = 0 ).
  ENDMETHOD.

  METHOD zif_dt_logger~length.
    rv_length = lines( get_message_handles( ) ).
  ENDMETHOD.

  METHOD zif_dt_logger~popup.
* See SBAL_DEMO_04_POPUP for ideas
  ...
  METHOD zif_dt_logger~save.
    CHECK settings->get_autosave( ) = abap_false.
    save_log( ).
  ENDMETHOD.

  METHOD zif_dt_logger~w.
    self = add(
      obj_to_log          = obj_to_log
      context             = context
      callback_form       = callback_form
  ...
  METHOD zif_dt_logger~set_header.
