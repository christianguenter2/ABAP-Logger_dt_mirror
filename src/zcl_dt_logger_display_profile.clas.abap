CLASS zcl_dt_logger_display_profile DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE
  ...
  PUBLIC SECTION.
    INTERFACES zif_dt_logger_display_profile.
  PROTECTED SECTION.
...
CLASS zcl_dt_logger_display_profile IMPLEMENTATION.
  ...
  METHOD zif_dt_logger_display_profile~get.
    r_display_profile = display_profile.
  ENDMETHOD.

  METHOD zif_dt_logger_display_profile~set.
    CASE abap_true.
      WHEN i_detlevel.
  ...
  METHOD zif_dt_logger_display_profile~set_context_message.
  ...
  METHOD zif_dt_logger_display_profile~set_context_tree.
  ...
  METHOD zif_dt_logger_display_profile~set_grid.
    zif_dt_logger_display_profile~set_value(
      i_fld = 'USE_GRID'
      i_val = i_grid_mode ).
  ...
  METHOD zif_dt_logger_display_profile~set_value.
    ...
    IF sy-subrc = 0.
      ...
      r_self = me.
    ELSE.
      RAISE EXCEPTION TYPE zcx_dt_logger_display_profile
        EXPORTING
          info = |field { i_fld } does not exist|.
    ENDIF.
