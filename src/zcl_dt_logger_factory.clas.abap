CLASS zcl_dt_logger_factory DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS create_log
      IMPORTING
        ...
        settings     TYPE REF TO zif_dt_logger_settings OPTIONAL
      RETURNING
        VALUE(r_log) TYPE REF TO zif_dt_logger.
    ...
    CLASS-METHODS create_settings
      RETURNING
        VALUE(r_settings) TYPE REF TO zif_dt_logger_settings.

    CLASS-METHODS create_collection
      RETURNING
        VALUE(r_collection) TYPE REF TO zif_dt_logger_collection.
    ...
    CLASS-METHODS create_display_profile
      ...
      RETURNING
        VALUE(r_display_profile) TYPE REF TO zif_dt_logger_display_profile.
  PROTECTED SECTION.
  PRIVATE SECTION.
ENDCLASS.


CLASS zcl_dt_logger_factory IMPLEMENTATION.

  METHOD create_log.
    FIELD-SYMBOLS <context_val> TYPE c.

    DATA lo_log TYPE REF TO zcl_dt_logger.
    ...
    ELSE.
      CREATE OBJECT lo_log->settings TYPE zcl_dt_logger_settings.
    ENDIF.
  ...
  METHOD create_settings.
    CREATE OBJECT r_settings TYPE zcl_dt_logger_settings.
  ENDMETHOD.
  ...
  METHOD open_log.
    ...
    IF sy-subrc = 1.
      IF create_if_does_not_exist = abap_true.
        r_log = zcl_dt_logger=>new( object    = object
                                 subobject = subobject
                                 desc      = desc ).
      ENDIF.
      RETURN.
  ...
  METHOD create_collection.
    CREATE OBJECT r_collection TYPE zcl_dt_logger_collection.
  ENDMETHOD.

  METHOD create_display_profile.
    CREATE OBJECT r_display_profile TYPE zcl_dt_logger_display_profile.
