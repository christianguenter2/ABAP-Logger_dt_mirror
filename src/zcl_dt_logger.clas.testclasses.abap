...
CLASS lcl_test DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA:
          anon_log     TYPE REF TO zif_dt_logger,
          named_log    TYPE REF TO zif_dt_logger,
          reopened_log TYPE REF TO zif_dt_logger.
...
CLASS lcl_test IMPLEMENTATION.
  ...
  METHOD setup.
    anon_log  = zcl_dt_logger=>new( ).
    named_log = zcl_dt_logger=>new( object = 'ABAPUNIT'
                                 subobject = 'LOGGER'
                                 desc = `Hey it's a log` ).
    reopened_log = zcl_dt_logger=>open( object = 'ABAPUNIT'
                                     subobject = 'LOGGER'
  ...
  METHOD can_create_expiring_log_days.
    DATA      expiring_log                  TYPE REF TO zif_dt_logger.
    DATA      act_header                    TYPE bal_s_log.
    CONSTANTS days_until_log_can_be_deleted TYPE i VALUE 365.

    expiring_log = zcl_dt_logger_factory=>create_log(
      object    = 'ABAPUNIT'
      subobject = 'LOGGER'
  ...
  METHOD can_open_or_create.
    DATA: created_log TYPE REF TO zif_dt_logger,
    ...
    created_log = zcl_dt_logger=>open( object = 'ABAPUNIT'
                                    subobject = 'LOGGER'
  ...
  METHOD can_add_log_context.
    DATA: log                 TYPE REF TO zif_dt_logger,
    ...
    log = zcl_dt_logger=>new( context = random_country_data ).
