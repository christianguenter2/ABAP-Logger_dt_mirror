CLASS zcl_dt_logger_collection DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE
  ...
  PUBLIC SECTION.
    INTERFACES: zif_dt_logger_collection.

  PROTECTED SECTION.
  PRIVATE SECTION.
    DATA:
      loggers TYPE STANDARD TABLE OF REF TO zif_dt_logger WITH DEFAULT KEY.
...
CLASS zcl_dt_logger_collection IMPLEMENTATION.

  METHOD zif_dt_logger_collection~add_logger.
    APPEND logger TO loggers.
  ENDMETHOD.

  METHOD zif_dt_logger_collection~display_logs.
    ...
    zif_dt_logger_collection~display_logs_using_profile( display_profile ).
  ENDMETHOD.

  METHOD zif_dt_logger_collection~display_logs_using_profile.
  ...
  METHOD get_log_handles.
    DATA logger TYPE REF TO zif_dt_logger.
