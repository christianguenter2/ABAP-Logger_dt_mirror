...
CLASS zcl_dt_logger_settings DEFINITION LOCAL FRIENDS lcl_logger_settings_should.
...
CLASS lcl_logger_settings_should DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_dt_logger_settings.
    METHODS setup.
...
CLASS lcl_logger_settings_should IMPLEMENTATION.
  ...
  METHOD have_correct_defaults.
    cl_abap_unit_assert=>assert_true(
      act     = cut->zif_dt_logger_settings~get_autosave( )
      msg     = |Auto save should be on by default| ).
    cl_abap_unit_assert=>assert_true(
      act     = cut->zif_dt_logger_settings~get_usage_of_secondary_db_conn( )
      msg     = |2nd database connection should be used by default| ).
    cl_abap_unit_assert=>assert_false(
      act     = cut->zif_dt_logger_settings~get_must_be_kept_until_expiry( )
      msg     = |Log should be deletable before expiry date is reached by default| ).
    cl_abap_unit_assert=>assert_initial(
      act     = cut->zif_dt_logger_settings~get_expiry_date( )
      msg     = |No expiry date set by default| ).
    cl_abap_unit_assert=>assert_equals(
      exp     = 10
      act     = cut->zif_dt_logger_settings~get_max_exception_drill_down( )
      msg     = |Max exception drill down should be 10 by default| ).
  ENDMETHOD.

  METHOD set_autosave.
    cut->zif_dt_logger_settings~set_autosave( abap_false ).
    cl_abap_unit_assert=>assert_false(
      act     = cut->zif_dt_logger_settings~get_autosave( )
  ...
  METHOD set_expiry_date.
    cut->zif_dt_logger_settings~set_expiry_date( '20161030' ).
    cl_abap_unit_assert=>assert_equals(
      exp     = '20161030'
      act     = cut->zif_dt_logger_settings~get_expiry_date( )
  ...
  METHOD set_expiry_in_days.
    cut->zif_dt_logger_settings~set_expiry_in_days( -1 ).
    cl_abap_unit_assert=>assert_initial(
      act     = cut->zif_dt_logger_settings~get_expiry_date( )
      msg     = |Expiry in days should remain default when setting incorrect values.| ).

    cut->zif_dt_logger_settings~set_expiry_in_days( 10 ).
  ...
  METHOD set_flag_to_keep_until_expiry.
    cut->zif_dt_logger_settings~set_must_be_kept_until_expiry( abap_true ).
    cl_abap_unit_assert=>assert_true(
      act     = cut->zif_dt_logger_settings~get_must_be_kept_until_expiry( )
  ...
  METHOD set_usage_of_2nd_db_connection.
    cut->zif_dt_logger_settings~set_usage_of_secondary_db_conn( abap_false ).
    cl_abap_unit_assert=>assert_false(
      act     = cut->zif_dt_logger_settings~get_usage_of_secondary_db_conn( )
  ...
  METHOD set_max_drilldown_level.
    cut->zif_dt_logger_settings~set_max_exception_drill_down( 20 ).
    cl_abap_unit_assert=>assert_equals(
      exp     = 20
      act     = cut->zif_dt_logger_settings~get_max_exception_drill_down( )
      msg     = |Setter for max drilldown level is not working correctly.| ).
    cut->zif_dt_logger_settings~set_max_exception_drill_down( -1 ).
    cl_abap_unit_assert=>assert_equals(
      exp     = 20
      act     = cut->zif_dt_logger_settings~get_max_exception_drill_down( )
      msg     = |Max exception drill down level should not change if value is incorrect.| ).
    cut->zif_dt_logger_settings~set_max_exception_drill_down( 0 ).
    cl_abap_unit_assert=>assert_equals(
      exp     = 0
      act     = cut->zif_dt_logger_settings~get_max_exception_drill_down( )
